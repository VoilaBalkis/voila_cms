...
if (isset($_GET['file']) && !checkRelativePath($_GET['file'])) {
    response(cbLang('filemanager.wrong path').AddErrorLocation())->send();
    exit;
}
...
if (isset($_GET['action'])) {
    switch ($_GET['action']) {
        case 'new_file_form':
            echo cbLang('filemanager.Filename') . ': <input type="text" id="create_text_file_name" style="height:30px"> <select id="create_text_file_extension" style="margin:0;width:100px;">';
        ...
        case 'view':
            if (isset($_GET['type'])) {
                ...
                response(cbLang('filemanager.view type number missing').AddErrorLocation())->send();
                exit;
            }
            break;
		...
		case 'save_img':
            ...
            if (preg_match('/^data:image\/(\w+);base64,/', $image_data, $type)) {
                ...
                if ($image_data === false) {
                    response(cbLang('filemanager.TUI_Decode_Failed').AddErrorLocation())->send();
                exit;
                }
            } else {
                response(cbLang('filemanager.').AddErrorLocation())->send();
                exit;
            }
            ...
            if (!checkresultingsize(strlen($image_data))) {
                response(sprintf(cbLang('filemanager.max_size_reached'), $config['MaxSizeTotal']).AddErrorLocation())->send();
                exit;
            }
            if ($ftp) {
                $temp = tempnam('/tmp', 'RF');
        ...
        case 'extract':
            if (!$config['extract_files']) {
                response(cbLang('filemanager.wrong action').AddErrorLocation())->send();
            }
            if ($ftp) {
            ...
            switch ($info['extension']) {
                case "zip":
                    ...
                    if ($zip->open($path) === true) {
                        ...
                        response(cbLang('filemanager.Zip_No_Extract').AddErrorLocation())->send();
                        exit;
                    }
                ...
                default:
                    response(cbLang('filemanager.Zip_Invalid').AddErrorLocation())->send();
                    exit;
            }
...
<?php endif;
        ...
        case 'copy_cut':
            ...
            $msg_sub_action = ($_POST['sub_action'] == 'copy' ? cbLang('filemanager.Copy') : cbLang('filemanager.Cut'));
            $path = $config['current_path'] . $_POST['path'];

            if (is_dir($path)) {
                // can't copy/cut dirs
                if ($config['copy_cut_dirs'] === false) {
                    response(sprintf(cbLang('filemanager.Copy_Cut_Not_Allowed'), $msg_sub_action, cbLang('filemanager.Folders')).AddErrorLocation())->send();
                    exit;
                }
                ...
                if ($config['copy_cut_files'] === false) {
                    response(sprintf(cbLang('filemanager.Copy_Cut_Not_Allowed'), $msg_sub_action, cbLang('filemanager.Files')).AddErrorLocation())->send();
                    exit;
                }
            }
        ...
        case 'chmod':
            if ($ftp) {
                ...
                if (
                    ...
                    response(sprintf(cbLang('filemanager.File_Permission_Not_Allowed'), (is_dir($path) ? cbLang('filemanager.Folders') : cbLang('filemanager.Files')), 403).AddErrorLocation())->send();
                    exit;
                }
            ...
            if ((!$ftp && is_dir($path))) {
                $ret .= '<div class="hero-unit" style="padding:10px;">'.cbLang('filemanager.File_Permission_Recursive').'<br/><br/>
                        <ul class="unstyled">
                            <li><label class="radio"><input value="none" name="apply_recursive" type="radio" checked> '.cbLang('filemanager.No').'</label></li>
                            <li><label class="radio"><input value="files" name="apply_recursive" type="radio"> '.cbLang('filemanager.Files').'</label></li>
                            <li><label class="radio"><input value="folders" name="apply_recursive" type="radio"> '.cbLang('filemanager.Folders').'</label></li>
                            <li><label class="radio"><input value="both" name="apply_recursive" type="radio"> '.cbLang('filemanager.Files').' & '.cbLang('filemanager.Folders').'</label></li>
                        </ul>
                        </div>';
            }
        ...
        case 'cad_preview':
            if ($ftp) {
                ...
                if (! file_exists($selected_file)) {
                    response(cbLang('filemanager.File_Not_Found').AddErrorLocation())->send();
                    exit;
                }
            }
            if ($ftp) {
        ...
        case 'get_file': // preview or edit
            ...
            if (! in_array($info['extension'], $allowed_file_exts)
                ...
                response(sprintf(cbLang('filemanager.File_Open_Edit_Not_Allowed'), ($sub_action == 'preview' ? strtolower(cbLang('filemanager.Open')) : strtolower(cbLang('filemanager.Edit')))).AddErrorLocation())->send();
                exit;
            }
